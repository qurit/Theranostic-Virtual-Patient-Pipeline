...
def runTOTSEG(input_path,output_path, totseg_para):
    ...
    outputs: 
        - ROI segmentation ml (single file) nifti file
        - Body segmentation nifti file
    ...
    if radiomics==True and ml==True:
        log.debug("Radiomics not supported with --ml; forcing ml=False so radiomics can run.")
        ml = False
    ...
    if os.path.isdir(input_path):  # DICOM folder
        log.info("DICOM folder being converting to NIfTI")
        print("[TOTSEG] DICOM folder being converting to NIfTI...")
        ...
        if low.endswith((".nii", ".nii.gz")):
            log.info("NIfTI file being copied into output folder")
            print("[TOTSEG] NIfTI file being copied into output folder...")
            sitk.WriteImage(sitk.ReadImage(input_path), ct_nifti, True)
        else:
            log.warn('Unsupported input. Provide a DICOM folder or a NIfTI (.nii/.nii.gz)')
            raise ValueError("[TOTSEG] Unsupported input. Provide a DICOM folder or a NIfTI (.nii/.nii.gz).")
        
        

    log.info(f'Using NIfTI: {ct_nifti}')
    ...
    log.debug(f"Output : {out_dir}")
    log.debug(f"Device : {device}")
    log.debug(f"Fast Mode : {fast}")
    log.debug(f"ROI Subset : {roi_subset}")
    log.debug(f"Ml(multilabel) mode : {ml}")
    log.debug(f"Statistics mode : {statistics}")
    log.debug(f"Radiomics mode : {radiomics}")
    ...
    if ml:
        ...
        log.info("Segmentating ROIs")
        print("[TOTSEG] Segmentating ROIs...")
        totalsegmentator(ct_nifti, ml_file, **kwargs)
        
        log.info("Segmentating Body")
        print("[TOTSEG] Segmentating Body...")
        totalsegmentator(ct_nifti, body_file, device=device, task="body",fast=fast)
        
        log.info("Segmentation complete (multilabel and body)")
        print("[TOTSEG] Segmentation complete (multilabel and body).")
        
        log.debug(f"Multilabel file: {ml_file}")
        log.debug(f"Body file: {body_file}")

        return ml_file,body_file
        
    else: #shouldnt be using this... mabye in future?
        # write the usual per-class directory structure
        #not using in my code
