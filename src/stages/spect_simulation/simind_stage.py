...
class SimindSimulationStage:
    ...
    def _copy_templates(self) -> None:  
        ...
        Required templates in <repo_root>/src/data:
        - scattwin.win  -> renamed to <prefix>.win
        - smc.smc       -> renamed to <prefix>.smc
        """  
        scatwin_file = os.path.join(self.repo_root, "src/data", "scattwin.win")
        shutil.copyfile(scatwin_file, os.path.join(self.work_dir, f"{self.prefix}.win"))

        smc_file = os.path.join(self.repo_root, "src/data", "smc.smc")
    ...
    def _run_jaszczak_calibration(self) -> None:  
        ...
        - Requires `jaszak.smc` to exist in <repo_root>/src/data.
        """  
        if self._calibration_exists():
            return

        jaszak_file = os.path.join(self.repo_root, "src/data", "jaszak.smc")
